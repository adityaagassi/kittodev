          							...
          							<table class="table" id="table2">
										<thead>
              							...
              							<tbody>
											...
											@endif
												<td>
													<a href="{{ url("completions/$completions->id") }}">{{ $completions->barcode_number }}</a>
												</td>
												<td>
													@if(isset($completions->material))
													<a href="{{ url("materials/$completions->material_id") }}">{{ $completions->material->material_number }}</a>
													@else
													-
													@endif
												</td>
												<td>
													@if(isset($completions->material))
													{{ $completions->material->description }}
													@else
													-
													@endif
												</td>
												<td>{{ $completions->location_completion }}</td>
												<td>{{ $completions->lot_completion }}</td>
												<td>{{ $completions->limit_used }}</td>
												<td>
													@if($completions->active == 1)
													Active
													@else
													Non Active
													@endif
												...
												<td>
													@if(isset($completions->user))
													{{ $completions->user->name }}
													@else
													-
													@endif
												</td>
												<td>{{ $completions->created_at }}</td>
												<td>{{ $completions->updated_at }}</td>
												<td class="center">
													<center>
														<a href="javascript:void(0)"  data-toggle="modal" data-target="#myModal" onclick="deleteConfirmation('{{ url("completions/$completions->id/delete") }}', '{{ $completions->barcode_number }}');">
															<span class="fa fa-times"></span>
														</a>
														&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
														&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
														<a href="{{ url("completions/$completions->id/edit") }}">
